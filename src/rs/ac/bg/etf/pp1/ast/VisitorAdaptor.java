// generated with ast extension for cup
// version 0.8
// 26/6/2025 18:20:58


package rs.ac.bg.etf.pp1.ast;

public abstract class VisitorAdaptor implements Visitor { 

    public void visit(OptionalNumConst OptionalNumConst) { }
    public void visit(FactorOpt FactorOpt) { }
    public void visit(Mulop Mulop) { }
    public void visit(GlobalVarItem GlobalVarItem) { }
    public void visit(DecType DecType) { }
    public void visit(FormParsOpt FormParsOpt) { }
    public void visit(InterfaceDecl InterfaceDecl) { }
    public void visit(DesignatorOrExprWhile DesignatorOrExprWhile) { }
    public void visit(Relop Relop) { }
    public void visit(OptionalMethodDecl OptionalMethodDecl) { }
    public void visit(TermList TermList) { }
    public void visit(Assignop Assignop) { }
    public void visit(InterfaceDeclOptList InterfaceDeclOptList) { }
    public void visit(MethodType MethodType) { }
    public void visit(CondTermList CondTermList) { }
    public void visit(DesignatorOpt DesignatorOpt) { }
    public void visit(FormParItem FormParItem) { }
    public void visit(OptionalExtends OptionalExtends) { }
    public void visit(StatementList StatementList) { }
    public void visit(FactorList FactorList) { }
    public void visit(Addop Addop) { }
    public void visit(Factor Factor) { }
    public void visit(CondTerm CondTerm) { }
    public void visit(VarList VarList) { }
    public void visit(CondFactList CondFactList) { }
    public void visit(DesignatorStatementWhile DesignatorStatementWhile) { }
    public void visit(DeclList DeclList) { }
    public void visit(Designator Designator) { }
    public void visit(ArrayOpt ArrayOpt) { }
    public void visit(OptionalExpr OptionalExpr) { }
    public void visit(FormParsList FormParsList) { }
    public void visit(ConstDeclList ConstDeclList) { }
    public void visit(ActParsList ActParsList) { }
    public void visit(IfCondition IfCondition) { }
    public void visit(InterfaceDeclOpt InterfaceDeclOpt) { }
    public void visit(VarDeclList VarDeclList) { }
    public void visit(Expr Expr) { }
    public void visit(OptionalActPars OptionalActPars) { }
    public void visit(DesignatorStatement DesignatorStatement) { }
    public void visit(OptionalCondition OptionalCondition) { }
    public void visit(OptionalDesignatorStatement OptionalDesignatorStatement) { }
    public void visit(Statement Statement) { }
    public void visit(VarDecl VarDecl) { }
    public void visit(ClassDecl ClassDecl) { }
    public void visit(CondFact CondFact) { }
    public void visit(Setop Setop) { }
    public void visit(MethodDeclList MethodDeclList) { }
    public void visit(GlobalVarList GlobalVarList) { }
    public void visit(DesignatorOrExpr DesignatorOrExpr) { }
    public void visit(Consts Consts) { }
    public void visit(FormPars FormPars) { }
    public void visit(OptionalElse OptionalElse) { }
    public void visit(Union Union) { visit(); }
    public void visit(Mod Mod) { visit(); }
    public void visit(Div Div) { visit(); }
    public void visit(Mull Mull) { visit(); }
    public void visit(Minus Minus) { visit(); }
    public void visit(Plus Plus) { visit(); }
    public void visit(Le Le) { visit(); }
    public void visit(Lt Lt) { visit(); }
    public void visit(Ge Ge) { visit(); }
    public void visit(Gt Gt) { visit(); }
    public void visit(Ne Ne) { visit(); }
    public void visit(Eq Eq) { visit(); }
    public void visit(Assign Assign) { visit(); }
    public void visit(Label Label) { visit(); }
    public void visit(DesignatorIdent DesignatorIdent) { visit(); }
    public void visit(DesignatorArr DesignatorArr) { visit(); }
    public void visit(DesignatorVar DesignatorVar) { visit(); }
    public void visit(ParenFactor ParenFactor) { visit(); }
    public void visit(NewFactor NewFactor) { visit(); }
    public void visit(BoolFactor BoolFactor) { visit(); }
    public void visit(CharFactor CharFactor) { visit(); }
    public void visit(NumConstFactor NumConstFactor) { visit(); }
    public void visit(DesignatorMethFactor DesignatorMethFactor) { visit(); }
    public void visit(DesignatorFactor DesignatorFactor) { visit(); }
    public void visit(UnaryMinusFactor UnaryMinusFactor) { visit(); }
    public void visit(NoUnaryFactor NoUnaryFactor) { visit(); }
    public void visit(TermMulFactors TermMulFactors) { visit(); }
    public void visit(TermFactor TermFactor) { visit(); }
    public void visit(Term Term) { visit(); }
    public void visit(AddTerms AddTerms) { visit(); }
    public void visit(AddTerm AddTerm) { visit(); }
    public void visit(MapExpretion MapExpretion) { visit(); }
    public void visit(Expretion Expretion) { visit(); }
    public void visit(ActParams ActParams) { visit(); }
    public void visit(NoActParams NoActParams) { visit(); }
    public void visit(ActPars ActPars) { visit(); }
    public void visit(FuncPars FuncPars) { visit(); }
    public void visit(NoFuncPars NoFuncPars) { visit(); }
    public void visit(Dec Dec) { visit(); }
    public void visit(Inc Inc) { visit(); }
    public void visit(FunctionCall FunctionCall) { visit(); }
    public void visit(DesignatorStmtError DesignatorStmtError) { visit(); }
    public void visit(UnionStmt UnionStmt) { visit(); }
    public void visit(DesignatorExpr DesignatorExpr) { visit(); }
    public void visit(Assignement Assignement) { visit(); }
    public void visit(DesignatorStmt DesignatorStmt) { visit(); }
    public void visit(Relops Relops) { visit(); }
    public void visit(NoRelops NoRelops) { visit(); }
    public void visit(AndConditions AndConditions) { visit(); }
    public void visit(ConditionFacts ConditionFacts) { visit(); }
    public void visit(ConditionTerm ConditionTerm) { visit(); }
    public void visit(OrConditions OrConditions) { visit(); }
    public void visit(ConditionTerms ConditionTerms) { visit(); }
    public void visit(Condition Condition) { visit(); }
    public void visit(DesignatorStmtErrorDoWhile DesignatorStmtErrorDoWhile) { visit(); }
    public void visit(UnionStmtDoWhile UnionStmtDoWhile) { visit(); }
    public void visit(DesignatorExprDoWhile DesignatorExprDoWhile) { visit(); }
    public void visit(AssignementDoWhile AssignementDoWhile) { visit(); }
    public void visit(DesignatorStmtDoWhile DesignatorStmtDoWhile) { visit(); }
    public void visit(DoWhileDesignatorStmt DoWhileDesignatorStmt) { visit(); }
    public void visit(NoDoWhileDesignatorStmt NoDoWhileDesignatorStmt) { visit(); }
    public void visit(DoWhileCond DoWhileCond) { visit(); }
    public void visit(NoDoWhileCond NoDoWhileCond) { visit(); }
    public void visit(PrintNumConst PrintNumConst) { visit(); }
    public void visit(NoPrintNumConst NoPrintNumConst) { visit(); }
    public void visit(ReturnExpr ReturnExpr) { visit(); }
    public void visit(NoReturnExpr NoReturnExpr) { visit(); }
    public void visit(BeginDoWhile BeginDoWhile) { visit(); }
    public void visit(While While) { visit(); }
    public void visit(MatchedStatements MatchedStatements) { visit(); }
    public void visit(DoWhile DoWhile) { visit(); }
    public void visit(Print Print) { visit(); }
    public void visit(Read Read) { visit(); }
    public void visit(Return Return) { visit(); }
    public void visit(Continue Continue) { visit(); }
    public void visit(Break Break) { visit(); }
    public void visit(If If) { visit(); }
    public void visit(DesignatorStmtt DesignatorStmtt) { visit(); }
    public void visit(IfError IfError) { visit(); }
    public void visit(IfCond IfCond) { visit(); }
    public void visit(Else Else) { visit(); }
    public void visit(WithElse WithElse) { visit(); }
    public void visit(NoElse NoElse) { visit(); }
    public void visit(Type Type) { visit(); }
    public void visit(FormPar FormPar) { visit(); }
    public void visit(FormParItemError FormParItemError) { visit(); }
    public void visit(FormParItemVar FormParItemVar) { visit(); }
    public void visit(FormParsListEnd FormParsListEnd) { visit(); }
    public void visit(FormParamsList FormParamsList) { visit(); }
    public void visit(FormParams FormParams) { visit(); }
    public void visit(Statements Statements) { visit(); }
    public void visit(NoStatements NoStatements) { visit(); }
    public void visit(VarDeclarations VarDeclarations) { visit(); }
    public void visit(NoVarDeclarations NoVarDeclarations) { visit(); }
    public void visit(MethodDecl MethodDecl) { visit(); }
    public void visit(Pars Pars) { visit(); }
    public void visit(NoPars NoPars) { visit(); }
    public void visit(ReturnVoid ReturnVoid) { visit(); }
    public void visit(ReturnType ReturnType) { visit(); }
    public void visit(MethodSignature MethodSignature) { visit(); }
    public void visit(Array Array) { visit(); }
    public void visit(NoArray NoArray) { visit(); }
    public void visit(VarItem VarItem) { visit(); }
    public void visit(VarListEnd VarListEnd) { visit(); }
    public void visit(VarListMore VarListMore) { visit(); }
    public void visit(VarDeclaration VarDeclaration) { visit(); }
    public void visit(GlobalVarError GlobalVarError) { visit(); }
    public void visit(GlobalVar GlobalVar) { visit(); }
    public void visit(GlobalVarListEnd GlobalVarListEnd) { visit(); }
    public void visit(GLobalVarListMore GLobalVarListMore) { visit(); }
    public void visit(GlobalVarDecl GlobalVarDecl) { visit(); }
    public void visit(ConstItem ConstItem) { visit(); }
    public void visit(ConstDeclarations ConstDeclarations) { visit(); }
    public void visit(ConstDeclaration ConstDeclaration) { visit(); }
    public void visit(BoolConst BoolConst) { visit(); }
    public void visit(CharConst CharConst) { visit(); }
    public void visit(NumConst NumConst) { visit(); }
    public void visit(ConstDecl ConstDecl) { visit(); }
    public void visit(MethodDeclarations MethodDeclarations) { visit(); }
    public void visit(NoMethodDeclarations NoMethodDeclarations) { visit(); }
    public void visit(VarDeclType VarDeclType) { visit(); }
    public void visit(ConstDeclType ConstDeclType) { visit(); }
    public void visit(ProgDeclList ProgDeclList) { visit(); }
    public void visit(NoProgDeclList NoProgDeclList) { visit(); }
    public void visit(ProgramName ProgramName) { visit(); }
    public void visit(Program Program) { visit(); }


    public void visit() { }
}
